...
try:
    from infra.integrations.facebook_instagram_login import (
...
try:
    from workers.analyzer.content_engine import analyze_and_recommend, get_recommendation_stats, get_inference_metrics
    from workers.analyzer.competitor_tracker import (
        analyze_competitor,
        get_market_insights,
        get_industry_trending_hashtags,
    )
    from workers.analyzer.strategist import generate_weekly_strategy
...
async def simple_upload_submit(
    ...
    try:
        ...
        if ANALYZER_AVAILABLE:
            try:
                from workers.analyzer.content_engine import analyze_and_recommend
